...
const toneOptions = ['Professional', 'Friendly', 'Luxury', 'Playful', 'Minimal', 'Bold', 'Elegant', 'Warm', 'Modern', 'Premium'];
...
export default function BrandKitPage() {
    ...
    const previewLogo = logo ? URL.createObjectURL(logo) : (brand?.logo_url ?? null);

    const brandDescription = useMemo(() => {
        const tone = form.brand_tone.length ? form.brand_tone.join(', ') : 'No tone selected';
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        form.brand_tone.forEach((tone: string) => payload.append('brand_tone[]', tone));
        ...
        <>
            ...
            <div className="space-y-6 p-4 md:p-6">
                <div className="rounded-2xl border bg-card p-6 shadow-sm">
                    <p className="text-sm font-medium text-muted-foreground">Brand Kit</p>
                    <h1 className="mt-2 text-3xl font-semibold tracking-tight">Brand Kit</h1>
                    <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
                        Manage the visual identity and brand voice used across your marketing content.
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="grid gap-6 lg:grid-cols-[1.5fr_0.9fr]">
                    <div className="space-y-6">
                            ...
                            <CardContent className="space-y-4">
                                <div className="flex flex-col gap-4 rounded-xl border border-dashed p-4 md:flex-row md:items-center">
                                    ...
                                    <div className="flex-1 space-y-2">
                                        ...
                                        />
                                        <p className="text-xs text-muted-foreground">Supported: JPG, PNG, WEBP. Max 2MB.</p>
                        ...
                        <Card className="shadow-sm">
                            <CardHeader>
                                <CardTitle className="text-xl">Brand voice</CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-5">
                                        ...
                                        {toneOptions.map((tone) => {
                                            ...
                                            return (
                                                <button
                                                    key={tone}
                                                    type="button"
                                                    onClick={() => setForm((current) => ({ ...current, brand_tone: toggleValue(current.brand_tone, tone) }))}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(event) => setForm((current) => ({ ...current, typography: event.target.value }))}
                                        placeholder="Modern Sans"
                                    />
                                    {errors?.typography && <p className="text-sm text-destructive">{errors.typography}</p>}
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Textarea
                                        ...
                                        onChange={(event) => setForm((current) => ({ ...current, brand_guidelines: event.target.value }))}
                                        rows={5}
                                        placeholder="Describe your writing rules, brand voice, and standards."
                                    />
                                    {errors?.brand_guidelines && <p className="text-sm text-destructive">{errors.brand_guidelines}</p>}
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Textarea
                                        ...
                                        onChange={(event) => setForm((current) => ({ ...current, visual_preferences: event.target.value }))}
                                        rows={5}
                                        placeholder="Describe your visual direction, lighting, compositions, and mood."
                                    />
                                    {errors?.visual_preferences && <p className="text-sm text-destructive">{errors.visual_preferences}</p>}
                                </div>
                            </CardContent>
                        </Card>
                    </div>
                    ...
                    <div className="space-y-6">
                        <Card className="shadow-sm">
                            ...
                            <CardContent className="space-y-4">
                                <div className="rounded-2xl border p-4" style={{ background: form.primary_color }}>
                                    <div className="rounded-xl border border-white/20 bg-white/10 p-4 backdrop-blur-sm">
                                        <div className="flex items-center justify-between gap-3">
                                            <div className="flex items-center gap-3">
                                                {previewLogo ? (
                                                    <img src={previewLogo} alt="Brand logo" className="h-12 w-12 rounded-lg object-cover" />
                                                ) : (
                                                ...
                                                <div>
                                                    <p className="text-sm font-medium text-white">{business?.name || 'Your business'}</p>
                                                    <p className="text-xs text-white/80">{brandDescription}</p>
                                                </div>
                                            </div>
                                            <div className="flex gap-2">
                                                <span className="h-4 w-4 rounded-full" style={{ backgroundColor: form.primary_color }} />
                                                <span className="h-4 w-4 rounded-full" style={{ backgroundColor: form.secondary_color }} />
                                                <span className="h-4 w-4 rounded-full" style={{ backgroundColor: form.accent_color }} />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="rounded-xl border bg-muted/40 p-4">
                                    <p className="text-xs uppercase tracking-wide text-muted-foreground">Sample heading</p>
                                    <h3 className="mt-2 text-2xl font-semibold" style={{ color: form.primary_color }}>
                                        Fresh, premium creative direction
                                    </h3>
                                    <p className="mt-3 text-sm text-muted-foreground" style={{ color: form.primary_color }}>
                                        {form.brand_guidelines || 'Describe the standards that should guide your visual storytelling.'}
                                    </p>
                                </div>

                                <div className="rounded-xl border p-4">
                                    <p className="text-xs uppercase tracking-wide text-muted-foreground">Sample body</p>
                                    <p className="mt-2 text-sm leading-6 text-foreground">
                                        {form.visual_preferences || 'Clean compositions, warm lighting, and product-first storytelling.'}
                                    </p>
                                </div>
                            </CardContent>
                        </Card>
